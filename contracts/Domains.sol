...
contract Domains is ERC721URIStorage {
  // OpenZeppelin gas-efficient counterse to keep track of tokenIds
  ...
  // `names` is a mapping of addresses to domain names
  ...
  constructor(string memory _tld) payable ERC721("Anon Domains", "ANOND") {
  ...
  function register(string calldata name) public payable {
    ...
    // Combine name passed into function with the TLD
    string memory _name = string(abi.encodePacked(name, ".", tld));

    // Create SVG image for the NFT with the name
    ...
    console.log("Registering %s.%s on the contract with tokenID %d", 
      name, tld, newRecordId);

    // Create the JSON metadata of our NFT. Combine strings and encoding as base64
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the Anon name service", "image": "data:image/svg+xml;base64,',
